 ...
 * @author John Sirach
 */
public final class Webclient_settingscategories extends Webservice_renderer {
    
    static Logger LOG = LogManager.getLogger(Webclient_settingscategories.class);
    
    @Override
    public void collect() {
        Map<String,Object> pageData = new HashMap<>();
        pageData.put("page_title", "Categories");
        try {
            pageData.put("categories", BaseCategories.getCategoryList());
        } catch (CategoriesException ex) {
            LOG.error("Could not load categories");
            pageData.put("categories", new ArrayList());
        }
        setData(pageData);
    }
}