 ...
 * @author John
 */
public final class SendPushbulletStatement extends AutomationStatement {

    List<TextAutomationVariable> texts;
    
    static org.apache.logging.log4j.Logger LOG = LogManager.getLogger(SendPushbulletStatement.class);
    
    public SendPushbulletStatement(){
        super("SendPushBulletStatement");
    }
    ...
    public boolean run() {
        ...
        try {
            LOG.info("Got pushbullet statement, sending message: {}", message.toString());
            MessengerPluginService.getInstance().sendPushbulletMessage(message.toString());
        } catch (MessengerException ex) {
            LOG.warn("Could not send pushbullet: {}", ex.getMessage());
        }
        return true;
    }
