 ...
 * @author John
 */
public class CommandDataFieldDetail extends DeviceCmd implements DeviceValueChangeListener {

    static Logger LOG = LogManager.getLogger(CommandDataFieldDetail.class);
    ...
    Label currentDataText = new Label();
    ...
    public CommandDataFieldDetail(Device device){
        this.device = device;
    }
    ...
    void buildLabel(){
        ...
        currentDataText.setText(setLabel + ": ");
        dragDroppable.getLabel().setText(prefix + device.getLastCmd(groupName, setName) + suffix);
        interfaceButton.getChildren().addAll(currentDataText,dragDroppable);
        if(device.getCommandGroups().get(groupName).getSetDetails(setName).containsKey("graph") && 
            ...
            */
            ...
            graphView.setOnMouseClicked((MouseEvent t) -> {
                ...
                try {
                    LOG.debug("Going to open graph window for {}, group: {}, set: {}",String.valueOf(device.getId()), groupName,setName);
                    graphObject = new DeviceGraphWindow(String.valueOf(device.getId()), groupName + ":" + setName);
                    WindowManager.openWindow(graphObject, t.getSceneX(), t.getSceneY());
                } catch (Exception ex) {
                    LOG.error("Could not open graph for: {} set: {}", device.getName(), setName, ex);
                }
            });
