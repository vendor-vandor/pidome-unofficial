...
public class LogarithmicAxis extends ValueAxis<Number> {
    ...
    private DoubleProperty logUpperBound = new SimpleDoubleProperty();
    private DoubleProperty logLowerBound = new SimpleDoubleProperty();
    ...
    protected List<Number> calculateMinorTickMarks() {
        ...
        if (range != null) {
            ...
            if(this.lowestValue < 0.001){
                minorTickMarksPositions.addAll(calculateTickMinorValuesPrivate(0.001, 0.0001,0.0009));
            }
            if(this.lowestValue < 0.01){
                minorTickMarksPositions.addAll(calculateTickMinorValuesPrivate(0.01, 0.001, 0.009));
            }
            if(this.lowestValue < 0.1){
                minorTickMarksPositions.addAll(calculateTickMinorValuesPrivate(0.1, 0.01, 0.09));
            }
            if(this.lowestValue < 1.0){
                minorTickMarksPositions.addAll(calculateTickMinorValuesPrivate(1.0, 0.1, 0.9));
            }
    ...
    protected List<Number> calculateTickValues(double length, Object range) {
        ...
        if (range != null) {
            ...
            if(this.lowestValue < 0.001){
                tickPositions.addAll(calculateTickMajorValuesPrivate(0.001, 0.0001,0.0009));
            }
            if(this.lowestValue < 0.01){
                tickPositions.addAll(calculateTickMajorValuesPrivate(0.01, 0.001, 0.009));
            }
            if(this.lowestValue < 0.1){
                tickPositions.addAll(calculateTickMajorValuesPrivate(0.1, 0.01, 0.09));
            }
            if(this.lowestValue < 1.0){
                tickPositions.addAll(calculateTickMajorValuesPrivate(1.0, 0.1, 0.9));
            }
    ...
    private List<Number> calculateTickMinorValuesPrivate(double modifiedRange, double plus, double endTick){
        List<Number> minorTickMarksPositions = new ArrayList<>();
        int minorTickMarkCount = getMinorTickCount();
        for (double i = 0; i <= modifiedRange; i += plus) {
            for (double j = plus; j <= endTick; j += (1. / minorTickMarkCount)) {
                double value = j * Math.pow(10, i);
                minorTickMarksPositions.add(value);
            }
        }
    ...
    private List<Number> calculateTickMajorValuesPrivate(double modifiedRange, double plus, double endTick){
        List<Number> tickPositions = new ArrayList<>();
        for (double i = 0; i <= modifiedRange; i += plus) {
            for (double j = plus; j <= endTick; j++) {
                double value = j * Math.pow(10, i);
                tickPositions.add(value);
            }
        }
    ...
    public Number getValueForDisplay(double displayPosition) {
        if(displayPosition==0.0) return 0.0;
