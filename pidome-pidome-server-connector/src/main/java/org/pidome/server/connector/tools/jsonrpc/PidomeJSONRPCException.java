...
public class PidomeJSONRPCException extends Exception {
    ...
    JSONError jsonError = JSONError.SERVER_ERROR;
    ...
    public final String getJsonReadyMessage(){
        ...
        StringBuilder exceptionAsString = new StringBuilder();
        if(loggerConfig.getLevel().isLessSpecificThan(Level.DEBUG)){
            exceptionAsString.append(", \"data\": { \"message\":\"").append(getMessage()).append("\", \"trace\": \"");
            StringWriter sw = new StringWriter();
            printStackTrace(new PrintWriter(sw));
            exceptionAsString.append(sw.toString()).append("\"}");
        } else {
            exceptionAsString.append(", \"data\": { \"message\":\"").append(getMessage()).append("\"}");
        }
        return "{\"jsonrpc\": \"2.0\", \"error\": {\"code\": "+jsonError.toLong()+", \"message\": \""+message+"\""+exceptionAsString.toString()+"}, \"id\": "+(stringId!=null?"\""+stringId+"\"":(setLongId==true?longId:"null"))+"}";
    }
