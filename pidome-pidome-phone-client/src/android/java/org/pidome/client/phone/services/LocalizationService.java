 ...
 * @author John
 */
public class LocalizationService implements LocalizationInfoInterface {
    ...
    protected LocalizationService(SystemService systemService, AndroidPreferences prefs){
    ...
    public final void setLocalizationPreferences(boolean enabled, long timeToWait, boolean wifiHomeEnabled){
        ...
        try {
            ...
            Logger.getLogger(LocalizationService.class.getName()).log(Level.SEVERE, "Could not store GPS preferences", ex);
        }
        restartGPSExecutor();
    }
    ...
    protected void restartGPSExecutor(){
        stopGPSExecutor();
        startGPSExecutor();
    }
    
    protected void startGPSExecutor(){
        if(prefs.getBoolPreference("gpsEnabled", false)){
            new Thread(){
                @Override
    ...
    protected void stopGPSExecutor(){
    ...
    protected final void sendCurrentLocation(){
        if(systemService.getSystem().getClient().isloggedIn()){
            new Thread(){
                @Override
                public final void run(){
                    try {
                        ...
                        PCCEntityDataHandler result = systemService.getSystem().getConnection().getJsonHTTPRPC("UserService.updateMyLocation", sendObject, "UserService.updateMyLocation");
                        ...
                        Logger.getLogger(LocalizationService.class.getName()).log(Level.SEVERE, ex.getMessage(), ex);
                    } catch (Exception ex){
                        Logger.getLogger(LocalizationService.class.getName()).log(Level.SEVERE, "Uncaught exception: " + ex.getMessage(), ex);
                    }
                }
            }.start();
        }
