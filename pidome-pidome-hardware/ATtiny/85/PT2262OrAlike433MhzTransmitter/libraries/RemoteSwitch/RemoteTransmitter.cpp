...
FlamingoSwitch::FlamingoSwitch(byte pin, unsigned int periodusec) : RemoteTransmitter(pin,periodusec,3) { 
    //Call contructor 
} 


void FlamingoSwitch::sendSignal(byte systemCode, char device, boolean on) {         
    sendTelegram(getTelegram(systemCode,device,on), _pin); 
} 

unsigned long FlamingoSwitch::getTelegram(byte systemCode, char device, boolean on) { 
    byte trits[12]; 
