...
final class FloorsViewPort3D extends FloorsViewManagement {
    ...
    public FloorsViewPort3D(FloorMapService floorMapService, double width, double height) {
    ...
    protected void buildScene() {
        PointLight highlight = new PointLight();
        for (FloorMapFloor floor : getFloorsAsList()) {
            try {
                ...
                group.addEventHandler(MouseEvent.MOUSE_PRESSED, (MouseEvent t) -> {
                    if (activeFloor == null) {
                        setFloorActive(group.getFloorId());
                ...
                Logger.getLogger(FloorsViewPort3D.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        paneHolder3D.getChildren().addAll(floorsList);
        paneHolder3D.getChildren().add(highlight);
        getChildren().remove(pleaseWait);
        getChildren().add(getSub3DScene());
        setStartPositions();
        showRegions(false, true);
        showRoomNames(false);
        if (DisplayTools.getUserDisplayType().equals(DisplayType.LARGE)) {
            SequentialTransition seqT = new SequentialTransition();
            /// Make sure the lowest floor is added first. Otherwise the floors move through eachother.
            List<Floor3DGroup> ordered = new ArrayList<>();
            for (Floor3DGroup floor : floorsList) {
                if(ordered.isEmpty()){
                    ordered.add(floor);
                } else {
                    int putPos = 0;
                    for(Floor3DGroup floorCheck:ordered){
                        if(floor.getFloorLevel()<floorCheck.getFloorLevel()){
                            break;
                        } else {
                            putPos++;
                        }
    ...
    final void setFloorActive(int floorId) {
        if (!animationBusy) {
            if (activeFloor == null) {
                activeFloor = getFloor3DGroupByFloorId(floorId);
                moveOutFromSet(floorId).play();
            } else if (activeFloor.getFloorId() != floorId) {
                SequentialTransition seqT = new SequentialTransition();
                seqT.getChildren().addAll(resetFloorsFromFocus(activeFloor), moveOutFromSet(floorId));
                seqT.play();
            }
            if(floorEnableListener!=null){
                floorEnableListener.setSelectedFloor(floorId);
            }
            if(floormoveEnabled!=null){
                floormoveEnabled.setFloorMoveEneabled();
            }
            animationBusy = true;
        }
    }

    private Floor3DGroup getFloor3DGroupByFloorId(int floorId){
        for(Floor3DGroup floor:floorsList){
    ...
    private Transition moveOutFromSet(int floorId) {
        ...
        Floor3DGroup floorToActivate = getFloor3DGroupByFloorId(floorId);
        
        for (Floor3DGroup floor : floorsList) {
            if (floor.getFloorLevel() < floorToActivate.getFloorLevel()) {
                parT.getChildren().add(floor.animateToPositionYAxis(height * 2));
            } else if (floor.getFloorLevel() > floorToActivate.getFloorLevel()) {
                parT.getChildren().add(floor.animateToPositionYAxis(-height * 2));
            }
        }
        ...
        double floorWidthDiff = (largestWidthGroup.getWidth() - floorToActivate.getWidth()) / 2;
        Transition trans = floorToActivate.animateToFront(-floorToActivate.getHeight() / 3, -30, -floorWidthDiff);
    ...
    final SubScene getSub3DScene() {
        ...
        PointLight defaultLight = new PointLight(Color.web("#eeeeee"));
