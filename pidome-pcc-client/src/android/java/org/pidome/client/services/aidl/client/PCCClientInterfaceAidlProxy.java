...
import org.pidome.client.system.SystemServiceInitDataListener;
...
import org.pidome.pcl.data.parser.jsonrpc.PidomeJSONRPC;
import org.pidome.pcl.data.parser.jsonrpc.PidomeJSONRPCException;

/**
 *
 * @author John
 */
public class PCCClientInterfaceAidlProxy implements PCCClientInterface,PCCConnectionNameSpaceRPCListener {
    
    private static String TAG = "PCCClientInterfaceAidlProxy";
     ...
     * Connection proxy.
     */
    private final PCCConnectionInterfaceAidlProxy connection;
     ...
     */
    public PCCClientInterfaceAidlProxy(LocalSettingsStorageInterface settings, LocalPreferenceStorageInterface preferences, PCCConnectionInterfaceAidlProxy connection){
        this.settings    = settings;
        this.connection  = connection;
        this.preferences = preferences;
        this.entities    = new Entities(this.connection);
        //// We are logged in. Lets get ths party going!
    ...
    public final boolean isloggedIn(){
        return this.connection.isClientLoggedIn();
    }
    ...
    private void sendInitPathsRequest(){
        try {
            ...
            Logger.getLogger(PCCClientInterfaceAidlProxy.class.getName()).log(Level.SEVERE, "Problem sending client init paths request", ex);
        } catch (PidomeJSONRPCException ex) {
            Logger.getLogger(PCCClientInterfaceAidlProxy.class.getName()).log(Level.SEVERE, "Could not create RPC message", ex);
        }
    }
    ...
    public void handleRPCCommandByResult(PCCEntityDataHandler rpcDataHandler) {
        try{
            switch((String)rpcDataHandler.getId()){
                case "SystemService.getClientInitPaths":
                    ...
                    try {
                        ...
                        boolean isSSL = this.connection.getConnectionData().getSocketHasSSL();
                        if(this.connection.getConnectionData().getHttpPort()==0){
                            if(isSSL){
                                provider.setHttpPort(((Number)result.get("httpsport")).intValue());
                            } else {
                                provider.setHttpPort(((Number)result.get("httpport")).intValue());
                            }
                        }
                        provider.setRPCLocation((String)result.get("jsonurl"));
                        provider.setIsSecure(isSSL);
                        provider.setXmlLocation(((String)result.get("initurl")).substring(0, ((String)result.get("initurl")).lastIndexOf("/")+1));
                        provider.setSuccess(true);
                        ((SystemServiceInitDataListener)this.connection).handleClientInitPaths(provider);
                    } catch (IllegalArgumentException ex){
                        Logger.getLogger(PCCClientInterfaceAidlProxy.class.getName()).log(Level.SEVERE, "Invalid init paths received (IllegalArgumentException): {1}", rpcDataHandler.getResult());
                        ...
                        Logger.getLogger(PCCClientInterfaceAidlProxy.class.getName()).log(Level.SEVERE, "Invalid init paths received (Other): {1}", rpcDataHandler.getResult());
                        EntityErrorBroadcaster.broadcastMessage("Initialization error", "Invalid init paths received: " + ex.getMessage(), ex);
                        provider.setSuccess(false);
                    }
                break;
            ...
            Logger.getLogger(PCCClientInterfaceAidlProxy.class.getName()).log(Level.SEVERE, "Got problem", ex);
        }
    }
    
    /**
     * Updates the current client status.
     * @param status 
     */
    protected final void updateCurrentClientStatusFromString(String status){
        for(PCCClientInterface.ClientStatus stat: PCCClientInterface.ClientStatus.values()){
    ...
    protected void broadcastLoginEventByString(String status, int errCode, String message){
        for(PCCClientInterface.ClientStatus stat: PCCClientInterface.ClientStatus.values()){
            if(stat.toString().equals(status)){
                broadcastLoginEvent(stat, errCode, message);
            }
        }
    }
