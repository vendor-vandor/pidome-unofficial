...
public class FileUtils {
    ...
    public static ArrayList<String> getTrainingFnms() // return all the names of the training image files + their paths
    {
        List<File> files = new ArrayList<>();
        File dirF = new File(TRAINING_DIR);
        for(File subDir:dirF.listFiles()){
            if(subDir.isDirectory()){
                files.addAll(Arrays.asList(subDir.listFiles(new FilenameFilter() {
        ...
        File[] fnms = files.toArray(new File[files.size()]);
    ...
    private static ArrayList<String> getPathNms(File[] fnms) {
        ArrayList<String> imFnms = new ArrayList<>();
        for (File fnm : fnms) {
            imFnms.add(fnm.getAbsolutePath());
        }
        Collections.sort(imFnms);
        System.out.println(imFnms);
        return imFnms;
    }  // end of getPathNms()

  // -------------------- image files input -------------------------------
    public static BufferedImage[] loadTrainingIms(ArrayList<String> fnms) /* load all the specified images, making sure they are grayscale and the 
     same dimensions */ {
        ...
        for (String fnm : fnms) {
            try {
                ...
                ims[i++] = im; //ImageUtils.toScaledGray(im, 1.0);    // make sure the images are gray
            } catch (Exception ex) {
                System.out.println("Could not read image from " + fnm + ", Reason: " + ex.getMessage());
            }
        }
    ...
    public static BufferedImage loadImage(String imFnm) // return an image
    {
    ...
    public static void saveImage(BufferedImage im, String fnm) // save image in fnm
    {
        try {
  ...
  // ------------------ FaceBundle I/O --------------------------
    ...
    public static void writeCache(FaceBundle bundle) // save the FaceBundle object in a file called EF_CACHE
    {
  ...
  // ------------------ save EigenFaces as images -------------------
    public static void saveEFIms(Matrix2D egfaces, int imWidth) /* save each row of the eigenfaces matrix as an image in EIGENFACES_DIR, 
    ...
    private static void makeDirectory(String dir) // create a new directory or delete the contents of an existing one
    {
    ...
    private static void saveArrAsImage(String fnm, double[] imData, int width) // save a ID array as an image
    {
  ...
  // -------------------- save reconstructed images ------------------------------
    public static void saveReconIms2(double[][] ims, int imWidth) /* save each row of the images array as a separate image in RECON_DIR, 
     whose pixel width is imWidth */ {
