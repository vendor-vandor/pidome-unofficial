...
import org.pidome.plugins.mqtt.pidomeMQTTBroker.FromBrokerToPiDomeInteral;
...
public class ProtocolProcessor implements EventHandler<ValueEvent> {
    ...
    private FromBrokerToPiDomeInteral pidomeListener;
    ...
    void init(SubscriptionsStore subscriptions, IMessagesStore storageService, 
        ...
        // New style (Ringbuffer 3.0(?)>). Still need to check if current Blocking wait is good performance and buffer sizes needs to be watched.
        //m_ringBuffer = RingBuffer.createSingleProducer(ValueEvent.EVENT_FACTORY, 1024 * 32, new BlockingWaitStrategy());

        /// Set to 256 items.
        m_ringBuffer = RingBuffer.createSingleProducer(ValueEvent.EVENT_FACTORY, 256, new BlockingWaitStrategy());
        
        SequenceBarrier barrier = m_ringBuffer.newBarrier();
        m_eventProcessor = new BatchEventProcessor<>(m_ringBuffer, barrier, this);
        //TODO in a presentation is said to don't do the followinf line!!
        m_ringBuffer.addGatingSequences(m_eventProcessor.getSequence());
        m_executor.submit(m_eventProcessor);
    }
    
    public final void addPiDomeListener(FromBrokerToPiDomeInteral pidomeListener){
        this.pidomeListener = pidomeListener;
    }
    
    public final void removeInternalListener(){
        this.pidomeListener = null;
    }
    ...
    /**
     * Added message processing from an internal process.
     * This is a custom implementation and will use addInFLight.
     * @param msg 
     */
    public final void processInternalPublish(PublishMessage msg){
        final String topic = msg.getTopicName();
        final AbstractMessage.QOSType qos = msg.getQos();
        final ByteBuffer message = msg.getPayload();
        boolean retain = msg.isRetainFlag();
    ...
    private void processPublish(String clientID, String topic, QOSType qos, ByteBuffer message, boolean retain, Integer messageID) { 
        LOG.debug("Publish recieved from clientID <{}> on topic <{}> with QoS {}", clientID, topic, qos);
        
        /// Added for internal processing to device mapping
        if(this.pidomeListener!=null)this.pidomeListener.handleFromExternal(clientID, topic, message);
        /// 
